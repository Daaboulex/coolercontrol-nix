    ...
    for dev in devs:
        ...
        if channels:
            ...
            temp_ch = [
                ...
                if not v.get("speed_options")
                and not v.get("lcd_info")
                and not v.get("lighting_modes")
            ]
            lcd_ch = [k for k, v in channels.items() if v.get("lcd_info")]
            light_ch = [k for k, v in channels.items() if v.get("lighting_modes")]
            if fan_ch:
                click.echo(
                    f"    Fan channels:      {len(fan_ch)} ({', '.join(fan_ch[:5])})"
                )
            if temp_ch:
                click.echo(
                    f"    Temp sensors:      {len(temp_ch)} ({', '.join(temp_ch[:5])})"
                )
            if lcd_ch:
                click.echo(
                    f"    LCD screens:       {len(lcd_ch)} ({', '.join(lcd_ch)})"
                )
            if light_ch:
                click.echo(
                    f"    Lighting channels: {len(light_ch)} ({', '.join(light_ch)})"
                )
        click.echo()
...
def devices_set_pwm(ctx, device_uid: str, channel: str, pwm_mode: int):
    ...
    api(
        "PUT",
        f"/devices/{device_uid}/settings/{channel}/pwm",
        ctx.obj["base"],
        json={"pwm_mode": pwm_mode},
    )
...
@click.pass_context
def devices_set_lighting(
    ctx, device_uid: str, channel: str, mode: str, color: tuple, speed: str | None
):
    ...
    api(
        "PUT",
        f"/devices/{device_uid}/settings/{channel}/lighting",
        ctx.obj["base"],
        json=payload,
    )
...
@click.argument("channel")
@click.option(
    "--mode", "-m", required=True, help="LCD mode (none, static, carousel, liquid-temp)"
)
@click.option("--brightness", "-b", type=int, help="LCD brightness 0-100")
@click.pass_context
def devices_set_lcd(
    ctx, device_uid: str, channel: str, mode: str, brightness: int | None
):
    ...
    api(
        "PUT",
        f"/devices/{device_uid}/settings/{channel}/lcd",
        ctx.obj["base"],
        json=payload,
    )
...
def devices_asetek690(ctx, device_uid: str, legacy: bool):
    ...
    api(
        "PATCH",
        f"/devices/{device_uid}/asetek690",
        ctx.obj["base"],
        json={"is_legacy690": legacy},
    )
    click.echo(
        f"AseTek 690 legacy mode {'enabled' if legacy else 'disabled'} for {device_uid}"
    )
